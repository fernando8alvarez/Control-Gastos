...
export default function BudgetControl({ budget, gastos, setGastos, setBudget, setIsValidBudget }) {
    ...
    return (
        <div className="contenedor-presupuesto contenedor sombra dos-columnas">
            ...
            <div className="contenido-presupuesto">
                <button type="button" className="reset-app" onClick={handleResetApp}>Reiniciar App</button>
                <p><span>Presupuesto: </span>{formatCount(budget)}</p>
                <p className={available < 0 ? 'negativo' : ''}><span>Disponible: </span>{formatCount(available)}</p>
                <p><span>Gastado: </span>{formatCount(spent)}</p>
            </div>
        </div>
    )
}
